...
def run_evaluation(command):
    """
    Worker function to run a single Innovus evaluation command.
    """
    case_name = os.path.basename(command[-1]).split('.')[0]
    print(f"Starting evaluation for: {case_name}")
    try:
...
def main():
    """
    Main function to batch run Innovus evaluation in parallel.
    """
    parser = argparse.ArgumentParser(
        description='Batch run Innovus evaluation for multiple cases in parallel.',
        formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument('init_enc_root', help='Root directory for init.enc files of all cases.')
    ...
    init_enc_root = os.path.abspath(args.init_enc_root)
    def_results_root = os.path.abspath(args.def_results_root)

    if not os.path.isdir(init_enc_root):
        print(f"Error: init.enc root directory not found at {init_enc_root}", file=sys.stderr)
        return 1
    ...
    evaluator_script = os.path.join(script_dir, 'innovus_evaluate.py')

    if not os.path.isfile(evaluator_script):
        print(f"Error: innovus_evaluate.py not found at {evaluator_script}", file=sys.stderr)
        return 1
    ...
    for case_dir_name in sorted(case_dirs):
        ...
        init_enc_path = os.path.join(init_enc_root, case_name, 'pnr', 'build', 'dbs', 'init.enc')
        if not os.path.isfile(init_enc_path):
            print(f"Warning: init.enc not found for case {case_name} at {init_enc_path}. Skipping.", file=sys.stderr)
            continue
        ...
        command = [sys.executable, evaluator_script, init_enc_path, def_file_path]
        commands_to_run.append(command)
        print(f"Queued evaluation for {case_name}: init={init_enc_path}, def={def_file_path}")

    if not commands_to_run:
        print("No valid cases found to evaluate.")
        return 0
        
    print(f"\nStarting parallel evaluation for {len(commands_to_run)} cases...")
